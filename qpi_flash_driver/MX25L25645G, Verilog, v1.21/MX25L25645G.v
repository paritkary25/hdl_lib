// *============================================================================================== 
// *
// *   MX25L25645G.v - 256M-BIT CMOS Serial Flash Memory
// *
...
// * Reference Doc: MX25L25645G REV.1.8,DEC.20,2019
// * Creation Date: @(#)$Date: 2020/05/28 09:54:41 $
// * Version      : @(#)$Revision: 1.21 $
// * Description  : There is only one module in this file
// *                module MX25L25645G->behavior model for the 256M-Bit flash
// *----------------------------------------------------------------------------------------------
...
// * Note 2:power setup time is tVSL = 3000_000 ns, so after power up, chip can be enable.
...
`define MX25L25645GM 16SOP
//`define MX25L25645GM2 8SOP
//`define MX25L25645GZ 8WSON
//`define MX25L25645GXD 24BGA 
//`define MX25L25645GXC 24BGA
...
// *============================================================================================== 
    ...
    /*----------------------------------------------------------------------*/
        ...
        `define CR_Default4_0     5'b00000      // configuration register default value
...
// *============================================================================================== 
    ...
    parameter   tSHQZ           = 8,        // CS High to SO Float Time [ns]
                ...
                tQVD            = 0.6,          // Data Output Valid Variation Time
                tBP             = 15_000,      //  Byte program time
                tSE             = 30_000_000,      // Sector erase time  
                tBE             = 380_000_000,      // Block erase time
                tBE32           = 180_000_000,    // Block 32KB erase time
                tCE             = 110_000,      // unit is ms instead of ns  
                ...
                tCE_FM          = 80_000,   // unit is ms instead of ns for factory mode
                tPP_FM          = 160_000,   // Program time for factory mode
                tW              = 40_000_000,       // Write Status time 
                tWPS            = 15_000,     // Write Protection Select time
                ...
                tVSL            = 3000_000;     // Time delay to chip select allowed
    ...
    specify
        specparam   tSCLK   = 8.3,    // Clock Cycle Time [ns]
                    fSCLK   = 120,    // Clock Frequence except READ instruction
                    tRSCLK  = 20,   // Clock Cycle Time for READ instruction
                    fRSCLK  = 50,   // Clock Frequence for READ instruction
                    tCH     = 3.75,      // Clock High Time (min) [ns]
                    tCL     = 3.75,      // Clock Low  Time (min) [ns]
                    tCH_R   = 7,      // Clock High Time (min) [ns]
                    tCL_R   = 7,      // Clock Low  Time (min) [ns]
                    tCH_4PP = 3.75,      // Clock High Time (min) [ns]
                    tCL_4PP = 3.75,      // Clock Low  Time (min) [ns]
                    tSLCH   = 3,    // CS# Active Setup Time (relative to SCLK) (min) [ns]
                    tCHSL   = 3,    // CS# Not Active Hold Time (relative to SCLK)(min) [ns]
                    tDVCL   = 2,      // SI Setup Time (min) [ns]
                    tCLDX   = 2,      // SI Hold  Time (min) [ns]
                    ...
                    tTSCLK  = 12.5,    // Clock Cycle Time for 2XI/O READ instruction
                    tTSCLK2 = `LVR ? 8.3 : 7.5,    // Clock Cycle Time for 2XI/O READ instruction
                    tTSCLK3 = 12.5,    // Clock Cycle Time for 2XI/O READ instruction
                    tTSCLK4 = `LVR ? 8.3 : 7.5,    // Clock Cycle Time for 2XI/O READ instruction
                    fTSCLK  = 80,    // Clock Frequence for 2XI/O READ instruction
                    fTSCLK2 = `LVR ? 120 : 133,    // Clock Frequence for 2XI/O READ instruction
                    fTSCLK3 = 80,    // Clock Frequence for 2XI/O READ instruction
                    fTSCLK4 = `LVR ? 120 : 133,    // Clock Frequence for 2XI/O READ instruction

                    tQSCLK  = 12.5,    // Clock Cycle Time for 4XI/O READ instruction
                    tQSCLK2 = 18.5,    // Clock Cycle Time for 4XI/O READ instruction
                    tQSCLK3 = `LVR ? 11.9 : 9.6,    // Clock Cycle Time for 4XI/O READ instruction
                    tQSCLK4 = `LVR ? 8.3 : 7.5,    // Clock Cycle Time for 4XI/O READ instruction
                    fQSCLK  = 80,    // Clock Frequence for 4XI/O READ instruction
                    fQSCLK2 = 54,  // Clock Frequence for 4XI/O READ instruction
                    fQSCLK3 = `LVR ? 84 : 104,  // Clock Frequence for 4XI/O READ instruction
                    fQSCLK4 = `LVR ? 120 : 133,  // Clock Frequence for 4XI/O READ instruction

                    tFSCLK   = `LVR ? 8.3 : 7.5,    // Clock Cycle Time for FASTREAD instruction
                    tFSCLK2  = `LVR ? 8.3 : 7.5,    // Clock Cycle Time for FASTREAD instruction
                    tFSCLK3  = `LVR ? 8.3 : 7.5,    // Clock Cycle Time for FASTREAD instruction
                    tFSCLK4  = `LVR ? 8.3 : 7.5,    // Clock Cycle Time for FASTREAD instruction
                    fFSCLK   = `LVR ? 120 : 133,   // Clock Frequence for FASTREAD instruction
                    fFSCLK2  = `LVR ? 120 : 133,   // Clock Frequence for FASTREAD instruction
                    fFSCLK3  = `LVR ? 120 : 133,   // Clock Frequence for FASTREAD instruction
                    fFSCLK4  = `LVR ? 120 : 133,   // Clock Frequence for FASTREAD instruction

                    tFDSCLK   = `LVR ? 8.3 : 7.5,    // Clock Cycle Time for DREAD instruction
                    tFDSCLK2  = `LVR ? 8.3 : 7.5,    // Clock Cycle Time for DREAD instruction
                    tFDSCLK3  = `LVR ? 8.3 : 7.5,    // Clock Cycle Time for DREAD instruction
                    tFDSCLK4  = `LVR ? 8.3 : 7.5,    // Clock Cycle Time for DREAD instruction
                    fFDSCLK   = `LVR ? 120 : 133,   // Clock Frequence for DREAD instruction
                    fFDSCLK2  = `LVR ? 120 : 133,   // Clock Frequence for DREAD instruction
                    fFDSCLK3  = `LVR ? 120 : 133,   // Clock Frequence for DREAD instruction
                    fFDSCLK4  = `LVR ? 120 : 133,   // Clock Frequence for DREAD instruction

                    tFQSCLK   = `LVR ? 8.3 : 7.5,    // Clock Cycle Time for QREAD instruction
                    tFQSCLK2  = `LVR ? 8.3 : 7.5,    // Clock Cycle Time for QREAD instruction
                    tFQSCLK3  = `LVR ? 8.3 : 7.5,    // Clock Cycle Time for QREAD instruction
                    tFQSCLK4  = `LVR ? 8.3 : 7.5,    // Clock Cycle Time for QREAD instruction
                    fFQSCLK   = `LVR ? 120 : 133,    // Clock Frequence for QREAD instruction
                    fFQSCLK2  = `LVR ? 120 : 133,   // Clock Frequence for QREAD instruction
                    fFQSCLK3  = `LVR ? 120 : 133,   // Clock Frequence for QREAD instruction
                    fFQSCLK4  = `LVR ? 120 : 133,   // Clock Frequence for QREAD instruction

                    tQDTRSCLK   = 18.5,    // Clock Cycle Time for FASTDDR4READ instruction
                    tQDTRSCLK2   = 18.5,    // Clock Cycle Time for FASTDDR4READ instruction
                    tQDTRSCLK3   = `LVR ? 14.3 : 12.5,    // Clock Cycle Time for FASTDDR4READ instruction
                    tQDTRSCLK4   = `LVR ? 11.9 : 10,    // Clock Cycle Time for FASTDDR4READ instruction
                    fQDTRSCLK   = 54,   // Clock Frequence for FASTDDR4READ instruction
                    fQDTRSCLK2   = 54,   // Clock Frequence for FASTDDR4READ instruction
                    fQDTRSCLK3   = `LVR ? 70 : 80,   // Clock Frequence for FASTDDR4READ instruction
                    fQDTRSCLK4   = `LVR ? 84 : 100,   // Clock Frequence for FASTDDR4READ instruction
...
//    time    tRES;
    ...
    initial begin
        ...
        Chip_EN         = 1'b0; 
        tQVD_Seed       = tQVD * 10;
        tQVD_random1    = {$random} % (tQVD_Seed);
        tQVD_random2    = {$random} % (tQVD_Seed);
        tQVD_Random1    = tQVD_random1/10;
        tQVD_Random2    = tQVD_random2/10;
    ...
    begin : memory_initialize
        ...
        SFDP_ARRAY[9'h55] =  8'h59;
        SFDP_ARRAY[9'h56] =  8'hdd;
        SFDP_ARRAY[9'h57] =  8'h00;
        SFDP_ARRAY[9'h58] =  8'h82;
        SFDP_ARRAY[9'h59] =  8'h9f;
        SFDP_ARRAY[9'h5a] =  8'h03;
        SFDP_ARRAY[9'h5b] =  8'hdb;
...
// *============================================================================================== 
    ...
    assign   WP_B_INT = (Status_Reg[6] == 1'b0 && !ENQUAD) ? ((WP === 1'b1 || WP === 1'b0) ? WP : 1'b1) : 1'b1;
...
`endif
    ...
    always @( SIO3_Reg or SIO2_Reg or SIO1_Reg or SIO0_Reg ) begin
        if ( SIO3_OUT_EN && WP_OUT_EN && SO_OUT_EN && SI_OUT_EN ) begin
            SIO3_Out_Reg <= #(tCLQV-tQVD_Random2) SIO3_Reg;
            SIO2_Out_Reg <= #(tCLQV-tQVD_Random1) SIO2_Reg;
            SIO1_Out_Reg <= #(tCLQV)              SIO1_Reg;
            SIO0_Out_Reg <= #(tCLQV-tQVD)         SIO0_Reg;
        end
...
`endif
                  ...
                  RDID:
                   ...
                   QPIID:
                    ...
                    WRLR:
                        begin
                            if ( !DP_Mode && !WIP && WEL && Norm_Array_Mode && WPSEL_Mode && Chip_EN && !Secur_Mode && ~HPM_RD && !EPSUSP && !ENQUAD ) begin
    ...
    always @ ( RST_Event ) begin
        ...
        else if ( WRLR_Mode || WR_WPSEL_Mode || WRSPB_Mode || WRSCUR_Mode ||  PP_4XIO_Mode || PP_1XIO_Mode ) begin
            #(tREADY2_P);
        end
        else if ( SE_4K_Mode || ESSPB_Mode ) begin
            #(tREADY2_SE);
        end
...
// *========================================================================================== 
    ...
    task update_array;
        ...
        begin
            ...
            program_time = (Byte_PGM_Mode) ? tBP : (Factory_Mode ? tPP_FM : tPP);
            Status_Reg[0]= 1'b1;
            Secur_Reg[5] = 1'b0;
            if ( write_protect(Address) == 1'b0 ) begin
                for ( i = 0; i < Dummy_Count; i = i + 1 ) begin
...
// *==============================================================================================
    ...
    wire SI_IN_EN_DTR_W;
    assign SI_IN_EN_DTR_W = SI_IN_EN & DDR_Read_Mode;
    wire SO_IN_EN_DTR_W;
    assign SO_IN_EN_DTR_W = SO_IN_EN & DDR_Read_Mode;
    wire WP_IN_EN_DTR_W;
    assign WP_IN_EN_DTR_W = WP_IN_EN & DDR_Read_Mode;
    wire SIO3_IN_EN_DTR_W;
    assign SIO3_IN_EN_DTR_W = SIO3_IN_EN & DDR_Read_Mode;
    ...
    specify
        ...
        $setup ( SI &&& ~CS, negedge SCLK &&& SI_IN_EN_DTR_W,  tDVCL );
        $hold  ( negedge SCLK &&& SI_IN_EN_DTR_W, SI &&& ~CS,  tCLDX );

        $setup ( SO &&& ~CS, posedge SCLK &&& SO_IN_EN_W,  tDVCH );
        $hold  ( posedge SCLK &&& SO_IN_EN_W, SO &&& ~CS,  tCHDX );

        $setup ( SO &&& ~CS, negedge SCLK &&& SO_IN_EN_DTR_W,  tDVCL );
        $hold  ( negedge SCLK &&& SO_IN_EN_DTR_W, SO &&& ~CS,  tCLDX );

        $setup ( WP &&& ~CS, posedge SCLK &&& WP_IN_EN_W,  tDVCH );
        $hold  ( posedge SCLK &&& WP_IN_EN_W, WP &&& ~CS,  tCHDX );
        $setup ( WP &&& ~CS, negedge SCLK &&& WP_IN_EN_DTR_W,  tDVCL );
        $hold  ( negedge SCLK &&& WP_IN_EN_DTR_W, WP &&& ~CS,  tCLDX );

        $setup ( SIO3 &&& ~CS, posedge SCLK &&& SIO3_IN_EN_W,  tDVCH );
        $hold  ( posedge SCLK &&& SIO3_IN_EN_W, SIO3 &&& ~CS,  tCHDX );
        $setup ( SIO3 &&& ~CS, negedge SCLK &&& SIO3_IN_EN_DTR_W,  tDVCL );
        $hold  ( negedge SCLK &&& SIO3_IN_EN_DTR_W, SIO3 &&& ~CS,  tCLDX );
    ...
    always @ ( negedge SCLK ) begin
       ...
       //tCH_4PP
        if ( T_SCLK_N - T_SCLK_P < tCH_4PP && PP_4XIO_Chk && T_SCLK_N > 0 )
            $fwrite (AC_Check_File, "minimum SCLK High time tCH=%f ns, tCH timing violation at %f \n", tCH_4PP, $realtime );
        //tDVCL
        if ( T_SCLK_N - T_SI < tDVCL && SI_IN_EN && DDR_Read_Mode && T_SCLK_N > 0 )
            $fwrite (AC_Check_File, "minimum Data SI setup time tDVCL=%f ns, tDVCL timing violation at %f \n", tDVCL, $realtime );
        if ( T_SCLK_N - T_SO < tDVCL && SO_IN_EN && DDR_Read_Mode && T_SCLK_N > 0 )
            $fwrite (AC_Check_File, "minimum Data SO setup time tDVCL=%f ns, tDVCL timing violation at %f \n", tDVCL, $realtime );
        if ( T_SCLK_N - T_WP < tDVCL && WP_IN_EN && DDR_Read_Mode && T_SCLK_N > 0 )
            $fwrite (AC_Check_File, "minimum Data WP setup time tDVCL=%f ns, tDVCL timing violation at %f \n", tDVCL, $realtime );
        if ( T_SCLK_N - T_SIO3 < tDVCL && SIO3_IN_EN && DDR_Read_Mode && T_SCLK_N > 0 )
            $fwrite (AC_Check_File, "minimum Data SIO3 setup time tDVCL=%f ns, tDVCL timing violation at %f \n", tDVCL, $realtime );
    end
    ...
    always @ ( SI ) begin
        ...
        #0;  
        //tCLDX
        if ( T_SI - T_SCLK_N < tCLDX && SI_IN_EN && DDR_Read_Mode && T_SI > 0 )
            $fwrite (AC_Check_File, "minimum Data SI hold time tCLDX=%f ns, tCLDX timing violation at %f \n", tCLDX, $realtime );
    end
    ...
    always @ ( SO ) begin
        ...
        #0;  
        //tCLDX
        if ( T_SO - T_SCLK_N < tCLDX && SO_IN_EN && DDR_Read_Mode && T_SO > 0 )
            $fwrite (AC_Check_File, "minimum Data SO hold time tCLDX=%f ns, tCLDX timing violation at %f \n", tCLDX, $realtime );
    end
    ...
    always @ ( WP ) begin
        ...
        #0;  
        //tCLDX
        if ( T_WP - T_SCLK_N < tCLDX && WP_IN_EN && DDR_Read_Mode && T_WP > 0 )
            $fwrite (AC_Check_File, "minimum Data WP hold time tCLDX=%f ns, tCLDX timing violation at %f \n", tCLDX, $realtime );
    end
    ...
    always @ ( SIO3 ) begin
        ...
        #0;  
        //tCLDX
       if ( T_SIO3 - T_SCLK_N < tCLDX && SIO3_IN_EN && DDR_Read_Mode  && T_SIO3 > 0 )
            $fwrite (AC_Check_File, "minimum Data SIO3 hold time tCLDX=%f ns, tCLDX timing violation at %f \n", tCLDX, $realtime );
    end
